...
# Install shadowsocks-libev, supervisor and socat
...
# Create supervisor config to run shadowsocks, socat and bot
...
command=socat TCP-LISTEN:5432,fork,reuseaddr TCP:%(ENV_POSTGRES_REAL_HOST)s:5432 \n\
autostart=true \n\
autorestart=true \n\
priority=5 \n\
stdout_logfile=/dev/stdout \n\
